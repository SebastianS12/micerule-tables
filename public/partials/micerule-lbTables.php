...
foreach($judgeCounter as $key=>$value){
  $results2[$key] += floor($results2[$key]*0.03*$value);
}
...
if(is_user_logged_in()){
  ...
  foreach($topTwenty as $key=> $value){
    if(isset($judgeCounter[$key])){$jPoints=($results[$key]+($results[$key]*0.03*$judgeCounter[$key]));}else {$jPoints = "";}
    $html .= "<tr>";
    $html .= "<td class = 'season-position'></td>";
    $html .= "<td class = 'resultCell'>".$key."</td>";
    $html .= "<td class = 'resultCell2'>".$results[$key]."</td>";
    $html .= "<td class = 'resultCell2'>".$judgeCounter[$key]."</td>";
