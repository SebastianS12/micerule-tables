...
class AbsenteesModel{
    public static function getAbsentees($eventPostID, $judgeName){
        ...
        return $wpdb->get_results("SELECT pen_number, class_index FROM sm1_micerule_show_entries ENTRIES
                                    ...
                                    INNER JOIN ".$wpdb->prefix."micerule_show_classes_indices INDICES ON REGISTRATIONS.location_id = INDICES.location_id AND REGISTRATIONS.class_name = INDICES.class_name AND REGISTRATIONS.age = INDICES.age
                                    INNER JOIN ".$wpdb->prefix."micerule_show_classes CLASSES ON INDICES.location_id = CLASSES.location_id AND INDICES.class_name = CLASSES.class_name
